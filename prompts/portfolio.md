# Portfolio Prompt

Copy this into Codex after cloning the repo:

```text
Turn this repository into a portfolio website for [name or studio].

Requirements:
- Keep it static HTML, CSS, and vanilla JavaScript only
- Keep GitHub Pages compatibility
- Use a clean modern responsive layout
- No external framework dependency

Please:
- Create a strong hero section
- Add a selected work grid with 3 to 6 projects
- Add concise case-study summaries with outcomes
- Add a services section and a contact section
- Update title, meta description, canonical URL, and social metadata
- Update favicon references and OG/Twitter image references for my brand
- Remove demo-domain references and keep the site ready for my custom domain
- Keep the code readable and simple
```
